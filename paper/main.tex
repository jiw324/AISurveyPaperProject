...
Large Language Models (LLMs) such as GPT-4, Claude, and Gemini have achieved remarkable capabilities in natural language understanding and generation \cite{brown2020language, openai2023gpt4, wolf2020transformers, hoffmann2022training, ouyang2022training}. These models are increasingly integrated into real-world applications including chatbots, code assistants, search engines, and automated customer service systems. However, their deployment introduces significant security risks, particularly through \textit{prompt injection attacks} \cite{perez2022ignore, liu2023prompt, wei2023jailbroken, greshake2024promptinject, zhao2024survey}.
...
Current defenses against prompt injection are largely heuristic-based (keyword filtering, input sanitization rules) or rely on prompt engineering techniques (instruction hierarchy, delimiter tokens) \cite{wei2023jailbroken, zhao2024survey}. However, these approaches are brittle and easily evaded through paraphrasing, obfuscation, or novel attack patterns \cite{zou2023universal, greshake2024promptinject, sun2024defending}.
...
\textbf{Attack Taxonomy:} Prompt injection attacks have been systematically studied by recent work \cite{perez2022ignore, liu2023prompt, greshake2024promptinject, zhao2024survey}. Common attack categories include goal hijacking (directly instructing the model to ignore previous instructions), context manipulation (embedding malicious instructions within seemingly legitimate data), and jailbreaking (using role-play or hypothetical scenarios to bypass safety guardrails) \cite{wei2023jailbroken, zou2023universal, sun2024defending}.

\textbf{Jailbreaking:} Wei et al. \cite{wei2023jailbroken} demonstrated that adversarial prompts can bypass safety training in aligned models through techniques such as prefix injection, role-playing scenarios, and obfuscation; similar adversarial suffixes are studied by Zou et al. \cite{zou2023universal}. These works show that even well-aligned models remain vulnerable to carefully crafted prompts that gradually manipulate the conversation context.

\textbf{Real-World Impact:} Recent incidents have demonstrated the severity of prompt injection attacks, including disclosure of system prompts and policy bypasses in deployed assistants \cite{greshake2024promptinject, ganguli2022red}. These attacks pose significant risks in high-stakes applications such as healthcare assistants, financial advisors, and autonomous agents.

\subsection{Adversarial Detection}

\textbf{Text Classification:} Transformer-based models have shown strong performance in adversarial text detection tasks, including spam detection, toxic comment classification, and fake news identification \cite{wolf2020transformers, schick2021self}. BERT and its variants remain standard baselines for text classification tasks, achieving state-of-the-art results through transfer learning from large-scale pre-training \cite{devlin2019bert, liu2019roberta, sanh2019distilbert, he2021deberta}.

\textbf{Perplexity-based Detection:} Recent work has proposed using perplexity scores to detect out-of-distribution prompts \cite{mitchell2023detectgpt}, with the intuition that adversarial prompts may exhibit unusual statistical properties. However, these methods suffer from high false positive rates on legitimate but unusual queries, limiting their practical deployment.

\textbf{Rule-based Defenses:} Current production systems primarily rely on keyword filtering and regular expression matching to detect malicious prompts. These approaches are fast and interpretable but easily evaded through paraphrasing, obfuscation, or novel attack formulations \cite{zou2023universal, greshake2024promptinject}.

\subsection{Research Gap}

While existing work has identified the prompt injection threat and proposed heuristic defenses, there is limited research on \textit{learned detection systems} that can generalize across attack types. Our work fills this gap by evaluating transformer-based classifiers on a hardened, paraphrased dataset and reporting realistic multiclass metrics, complementing censorship-style defenses \cite{gao2023llm} and robustness surveys \cite{zhao2024survey}. Unlike prior work that focuses on specific attack types or heuristic defenses, we demonstrate that learned models can achieve competitive accuracy on harder data while exposing remaining robustness gaps.
...
\textbf{Two-stage hardening.} Inspired by recent red-teaming and robustness work \cite{ganguli2022red, greshake2024promptinject, sun2024defending}:
\begin{enumerate}
...
\textbf{4. Attack Type Difficulty:} Goal hijacking attacks (explicit instruction override) are easiest to detect, while obfuscated attacks (character-level transformations) are most challenging \cite{zou2023universal}. This suggests that detection systems should be augmented with preprocessing steps to normalize obfuscation before classification.
...
\textbf{6. Interpretability Insights:} Models primarily attend to imperative verbs, instruction negation phrases, and role manipulation cues. This suggests that augmenting models with explicit syntactic features (e.g., dependency parsing for imperative detection) could further improve performance, aligning with attribution analyses \cite{sundararajan2017axiomatic, voita2019analyzing}.
...
This work demonstrates that transformer-based classifiers are effective for detecting prompt injection attacks once the data is realistically difficult: DistilBERT reaches 83.5\% accuracy and 63.1\% macro-F1 on a hard, paraphrased, six-way dataset. Our planned ablation (encoder freezing vs. full fine-tuning) will isolate how much gain comes from task adaptation. Robustness to adaptive attackers \cite{wei2023jailbroken, wallace2019universal, greshake2024promptinject} remains open, so detection should be paired with LLM-level defenses and input sanitization \cite{gao2023llm, sun2024defending}.
