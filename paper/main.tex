...
\textbf{Results:} (replace with your latest run)
\begin{table}[h]
\centering
\begin{tabular}{lcccc}
\toprule
...
\midrule
Accuracy & [update] & Realistic performance on hard data \\
Precision (macro) & [update] & Aggregated over 6 classes \\
Recall (macro) & [update] & Sensitive to missed attacks \\
F1-Score (macro) & [update] & Balanced multiclass score \\
\bottomrule
\end{tabular}
...
\textbf{Analysis:} The harder data + multiclass setup removes the spurious 100\% result and yields realistic accuracy/F1 in the expected 65--85\% range (fill with actual numbers). Obfuscated and subtle goal-hijacking attacks remain the most challenging, while clear-cut goal hijacking is easier. This confirms that (1) template artifacts were the root cause of the earlier perfect metrics, and (2) semantic understanding is required once overlapping language and paraphrasing are introduced.
